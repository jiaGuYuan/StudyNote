...
def change_file_charset(file_name, output_file_name, charset, match_charset=''):
    try:
        ...
        if old_charset == charset:
            return 0
        print('ddd',old_charset)
        with open(file_name, 'r') as f:#with open(file_name, 'r', encoding=old_charset) as f:
            s = f.read()
        #s = s.decode(old_charset) #将原字编码文件的字符转码为unicode（中间码）
        print('ddd',old_charset)
...
def Do(dirname,toEncode, match_charset=''):
    ...
    elif(ChangeEncode(dirname,toEncode, match_charset)!=0):
        print ("[转换失败qq:] "+dirname)
    else:
        print ("[成功：] "+dirname)
