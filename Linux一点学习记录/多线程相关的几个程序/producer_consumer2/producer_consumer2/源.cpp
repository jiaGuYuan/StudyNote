/* 二个生产者、二个消费者，N个缓冲区时。
（使消费者按照生产者生产的顺序取出产品: 这种情况下只需要对生产者与消费者之间同步，消费者相互之间互斥，生产者相互之间互斥，
...
CRITICAL_SECTION g_csConsumerCode, g_csProducerCode;	//关键区用于互斥操作
...
DWORD WINAPI producer(LPVOID lpPara) //生产者
{
	...
	while(1)
	{
		...
		EnterCriticalSection(&g_csProducerCode); //进入临界区开始操作操作资源
			if (i == g_yield )
			{
				if (!end_producer)
				{
				...
				end_producer = 1;
				LeaveCriticalSection(&g_csProducerCode); //离开临界区
...
DWORD WINAPI consumer(LPVOID lpPara) //消费者
{
	while (1)
	{
		...
		EnterCriticalSection(&g_csConsumerCode); //进入临界区开始操作操作资源
		if (g_Buffer[g_x] == g_yield)
		{
			...
			end_consumer = 1;
			LeaveCriticalSection(&g_csConsumerCode); //离开临界区
...
int main()
{
	...
	InitializeCriticalSection(&g_csConsumerCode);	//初始化临界区  
	InitializeCriticalSection(&g_csProducerCode);
	...
	thread_handle[1] = CreateThread(NULL, 0, producer, NULL, 0, NULL);

	//内核对象的安全属性为默认设置、线程空间为默认大小、线程入口为函数为producer，不向线程函数传递参数，不返回该线程ID号
	thread_handle[2] = CreateThread(NULL, 0, consumer, NULL, 0, NULL);
	thread_handle[3] = CreateThread(NULL, 0, consumer, NULL, 0, NULL);
	...
	DeleteCriticalSection(&g_csConsumerCode);	//销毁临界区  
	DeleteCriticalSection(&g_csProducerCode);
